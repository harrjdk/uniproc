...
class MathSqrtOperator: Operator("SQRT") {
    override fun operation(vm: Vm, tokens: List<Token>, verbose: Boolean): List<Token> {
        if (verbose) {
            println("[DEBUG] entering mathematic square root operation")
        }
        if (tokens.isEmpty()) {
        ...
        output = if (rightSide.type==MathTypes.FLOAT.string) {
            sqrt(rightSide.value.toDouble()).toString()
        } else {
            sqrt(rightSide.value.toDouble()).toInt().toString()
        }
