...
class MathSinOperator: Operator("SIN") {
    override fun operation(vm: Vm, tokens: List<Token>, verbose: Boolean): List<Token> {
        if (verbose) {
            println("[DEBUG] entering mathematic sine operation")
        }
        if (tokens.isEmpty()) {
        ...
        output = if (rightSide.type==MathTypes.FLOAT.string) {
            sin(rightSide.value.toDouble()).toString()
        } else {
            sin(rightSide.value.toDouble()).toInt().toString()
        }
