...
class MathTanOperator: Operator("TAN") {
    override fun operation(vm: Vm, tokens: List<Token>, verbose: Boolean): List<Token> {
        if (verbose) {
            println("[DEBUG] entering mathematic tangent operation")
        }
        if (tokens.isEmpty()) {
        ...
        output = if (rightSide.type==MathTypes.FLOAT.string) {
            tan(rightSide.value.toDouble()).toString()
        } else {
            tan(rightSide.value.toDouble()).toInt().toString()
        }
