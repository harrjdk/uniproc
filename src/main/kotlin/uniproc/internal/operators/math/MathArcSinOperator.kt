...
class MathArcSinOperator: Operator("ASIN") {
    override fun operation(vm: Vm, tokens: List<Token>, verbose: Boolean): List<Token> {
        if (verbose) {
            println("[DEBUG] entering mathematic arcsine operation")
        }
        if (tokens.isEmpty()) {
        ...
        output = if (rightSide.type==MathTypes.FLOAT.string) {
            asin(rightSide.value.toDouble()).toString()
        } else {
            asin(rightSide.value.toDouble()).toInt().toString()
        }
