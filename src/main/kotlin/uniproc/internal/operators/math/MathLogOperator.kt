...
class MathLogOperator: Operator("LOG") {
    override fun operation(vm: Vm, tokens: List<Token>, verbose: Boolean): List<Token> {
        if (verbose) {
            println("[DEBUG] entering mathematic logarithmic operation")
        }
        if (tokens.isEmpty()) {
        ...
        if (valueBuffer!=NULL) {
            ...
            if (leftSide.type==MathTypes.FLOAT.string || rightSide.type==MathTypes.FLOAT.string) {
                output = log(leftSide.value.toDouble(), rightSide.value.toDouble()).toString()
            } else {
                output = log(leftSide.value.toDouble(), rightSide.value.toDouble()).toInt().toString()
            }
