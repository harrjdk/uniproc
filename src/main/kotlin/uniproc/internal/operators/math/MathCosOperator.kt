...
class MathCosOperator: Operator("COS") {
    override fun operation(vm: Vm, tokens: List<Token>, verbose: Boolean): List<Token> {
        if (verbose) {
            println("[DEBUG] entering mathematic cosine operation")
        }
        if (tokens.isEmpty()) {
        ...
        output = if (rightSide.type==MathTypes.FLOAT.string) {
            cos(rightSide.value.toDouble()).toString()
        } else {
            cos(rightSide.value.toDouble()).toInt().toString()
        }
