...
class MathArcTanOperator: Operator("ATAN") {
    override fun operation(vm: Vm, tokens: List<Token>, verbose: Boolean): List<Token> {
        if (verbose) {
            println("[DEBUG] entering mathematic arctangent operation")
        }
        if (tokens.isEmpty()) {
        ...
        output = if (rightSide.type==MathTypes.FLOAT.string) {
            atan(rightSide.value.toDouble()).toString()
        } else {
            atan(rightSide.value.toDouble()).toInt().toString()
        }
